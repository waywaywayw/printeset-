...
class spiderWay_picDownloader :
    ...
    def work(self, givenURL) :
        ...
        # logging.info('对应的album name is ' + scannedAlbum)
        # album是资源存放的具体文件夹名
        ...
        try :
            ...
            logging.info('指定目录Rootpath 不存在啊不存在！')
        ...
        Pins = soup.select('span > b')
        if Pins is not None:
            picNum = getPins(Pins[0].get_text())
        else:
            logging.info('在缓存源代码中没找到Pins 属性?')
            picNum = 0
            exit(1)

        # 4 通过页面源代码和picNum获取资源列表..
        picList = self.getPicListByPageSource(PageSource, picNum)
        logging.info('需要的图片数量 :'+str(len(picList)))
        # logging.info([pic for pic in picList])

        # 5 去重API
        self.historyData.removeDuplicate(picList)

        # 6 下载API
        logging.info('准备正式下载资源.. 需要下载的资源数量 : %d' %len(picList))
        if len(picList) !=0 :
            logging.info('\n............................................')
            ...
            logging.info('............................................')
            logging.info('...总共下载成功'+str(succ)+'张图片 / 下载失败'+str(fail)+'张图片...')
        else :
            logging.info('唔.. 不需要进行下载了..')
    ...
    def getPageSource(self, givenURL) :
        ...
        while True :
            ...
            if Pins is not None :
                ...
                if cnt !=0 and cnt %1 == 0 :
                    ...
                    time.sleep(1)
                    logging.info('滑一下.. 然后等待1秒...')
                    cnt += 1

        logging.info('album picNum : '+str(picNum))
        ...
        while True:
            ...
            # 统计当前页面的资源数量
            cur_len = len(list)
            logging.info('scanned pic resource : ' + str(cur_len))
        ...
        logging.info('网页源代码一缓存只路径 : '+os.path.join(self.DBpath, self.album+'_page_source.html'))
    ...
    def getPicListByPageSource(self, pageSource, picNum=-1) :
        ...
        # logging.info("从源代码中扫描到的图片资源数量 : "+str(len(_pictList)))
        if picNum >= 1000 :
            _pictList = _pictList[:len(_pictList)]
        else :
            _pictList = _pictList[:picNum]
        # logging.info("Just need pic resources : "+str(picNum))
        for pic in _pictList:
            ...
            # print(textObj)
            if imgObj is None:
                logging.info('没找到图片对象..?')
                continue
            else :
                __pic = _pic()
                # 获取图片链接...（不同站点需要修改的部分--------------------------------------------------------）
                __pic.URL = imgObj.find('img').attrs['src']
                # logging.info("imgURL :"+__pic.URL)
                # 获取更高分辨率的图片链接
...
if __name__ == '__main__':
    ...
    with open('givenURL.txt', 'r') as infile :
        for givenURL in infile.readlines() :
            logging.info('---------------------------------------------------------------------------------------------------')
            logging.info('givenURL is '+givenURL)
